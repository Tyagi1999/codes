...
# the node at index. If using a language that has no pointers (such as Python),
...
class Node(object):
    def __inint__(self, val=None, both=None):
        self.val=val
        self.both=None
...
class XOR_List(object):
    ...
    def add(self,element):
         if length==0:
             ...
             prev=derefrence_pointer(self.tail.both)
    ...
    def get(self,index):
        if index>=length:
            ...
            while index>0:
                node=derefrence_pointer(ptr)
                ptr=node.both^prev_addr
                prev_addr=ptr
                index-=1

            if index==0:
                return derefrence_pointer(ptr)
